...
func Generate(specPath string, cfg Config) (string, error) {
	...
	if err := runJSONSchemaInProcess(cfg.Package, initialisms, tmpOut, schemaPaths, cfg.SchemaPackages, discardDir, cfg.OmitValidation); err != nil {
		return "", err
	}
...
//
// `imports` are the additional imports the appended sections need
// (e.g. context/encoding/json/fmt for the publisher block). They get
// unioned with whatever go-jsonschema emitted and any aliases inside
// section sources.
func combineSections(jsonschemaSrc string, imports []string, sections []string, pkg string) ([]byte, error) {
	pkgLine := "package " + pkg
