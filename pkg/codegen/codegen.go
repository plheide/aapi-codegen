...
func combineSections(jsonschemaSrc string, imports []string, aliasedImports [][2]string, sections []string, pkg string) ([]byte, error) {
	...
	// Strip free-standing comments that the import rewrite orphaned:
	// template section dividers, godoc on top-level decls that landed
	// between the now-removed imports and the first real decl. Their
	// positions are no longer alignable by the printer — they end up
	// sandwiched inside the new consolidated import block.
	//
	// Keep exactly the FIRST file-level comment though: that's the
	// "Code generated by ..., DO NOT EDIT." marker tools and humans
	// look for at the top of generated files. Removing it would break
	// gofmt's go-generated-file detection too.
	//
	// Decl-bound doc comments survive either pass — they're attached
	// via the decl's Doc field, not file.Comments.
